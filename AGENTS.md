...
- **CI policy:** yalc is a strictly local-development mechanism. CI, when introduced, installs against published versions and does not attempt to resolve `file:.yalc/…` links on its own; it does not check out the upstream `plugable-express`, `liq-projects`, or `http-smart-response` repositories. A CI runner has neither those repos nor the developer's global yalc store, so `.yalc/` cannot be regenerated there from nothing — it must be provisioned (e.g. copied in from a pre-populated checkout, as `scripts/provision-local-deps.sh` does for a local task worktree) before `bun install` can succeed. `bun.lock` currently resolves three packages via `file:.yalc/…`: two direct dependencies, `@liquid-labs/plugable-express` and `@liquid-labs/liq-projects`, plus one transitive dependency, `@liquid-labs/http-smart-response`, pulled in by `@liquid-labs/plugable-projects-audit`'s own pinned dependency on it at its current version. This set is version-sensitive — a future dependency bump can add or remove a `file:.yalc/…` resolution — so treat it as a snapshot, not a permanent list; re-derive it with `grep -n 'file:\.yalc' bun.lock` when in doubt, and keep `scripts/provision-local-deps.sh`'s own `REQUIRED_YALC_PACKAGES` list in sync.
