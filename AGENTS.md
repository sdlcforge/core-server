...
- `src/lib/builtin-plugins.mjs` — aggregates all seven built-in (in-tree) submodules, in DAG (dependency) order — `credentials`, `projects`, `orgs`, `controls`, `issues-github` (directory `integrations-issues-github`), `work`, `projects-audit` — into the single plugin module registered via `plugable-express`'s `builtinPlugins` option. The order is load-bearing (fixes absorbed routes' positions and satisfies same-phase `app.ext` dependencies between submodules); see the file's own header comment before reordering or appending. `src/lib/test/host-declaration.test.js` asserts this array's exported `componentNames` against `package.json`'s declared `plugable.host.builtins[0].components` order element-for-element, so the two cannot silently drift apart.
- `src/controls/`, `src/credentials/`, `src/integrations-issues-github/`, `src/orgs/`, `src/projects/`, `src/projects-audit/`, `src/work/` — built-in (in-tree) plugin submodules, siblings of `src/lib/` and `src/cli/`. `import/no-restricted-paths` (in the root `.eslintrc.cjs`) forbids importing across any two of these seven directories directly — cross-submodule coordination goes through `app.ext`, never a static import; `src/lib/test/component-boundary.test.js` guards against that rule going silently inert.
...
- **Component boundary is lint-enforced.** A root `.eslintrc.cjs` adds an `import/no-restricted-paths` rule with one zone per built-in submodule (`src/controls/`, `src/credentials/`, `src/integrations-issues-github/`, `src/orgs/`, `src/projects/`, `src/projects-audit/`, `src/work/`), forbidding a direct import from one into another. Cross-component coordination is `app.ext`-only, matching what both `package.json`'s host manifest and the absorbed components' own former plugin manifests already declared. `src/lib/builtin-plugins.mjs`'s own aggregation imports are permitted structurally (the rule targets the seven component directories, not `src/lib/`), and `src/lib/test/component-boundary.test.js` is a Jest guard against the rule going silently inert (e.g. from a future ESLint config change), not against a real violation existing today — there are none.
...
- **CI policy:** yalc is a strictly local-development mechanism. CI, when introduced, installs against published versions and does not attempt to resolve `file:.yalc/…` links on its own; it does not check out the upstream `plugable-express` repository. A CI runner has neither that repo nor the developer's global yalc store, so `.yalc/` cannot be regenerated there from nothing — it must be provisioned (e.g. copied in from a pre-populated checkout, as `scripts/provision-local-deps.sh` does for a local task worktree) before `bun install` can succeed. `bun.lock` currently resolves exactly one package via `file:.yalc/…`, the direct dependency `@liquid-labs/plugable-express` — `@sdlcforge/dev-core` was absorbed in-tree as four of the seven [built-in components](#code-organization) and is no longer a dependency at all; no transitive `file:.yalc/…` resolution remains either. This set is version-sensitive — a future dependency bump can add or remove a `file:.yalc/…` resolution — so treat it as a snapshot, not a permanent list; re-derive it with `grep -n 'file:\.yalc' bun.lock` when in doubt, and keep `scripts/provision-local-deps.sh`'s own `REQUIRED_YALC_PACKAGES` list in sync.
