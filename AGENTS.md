...
- `createOrUpdatePullRequest` currently imports `determineCurrentMilestone` from the external `@liquid-labs/liq-projects-lib` package rather than implementing milestone determination locally; this is a known, tracked external dependency (see [docs/liq-integrations-issues-github-spec.md](./docs/liq-integrations-issues-github-spec.md#constraints-and-assumptions)), not an oversight — do not silently work around it.
...
- `@liquid-labs/liq-projects-lib` — supplies `determineCurrentMilestone`, imported directly by `create-or-update-pull-request.mjs` (see [Conventions](#conventions)).
