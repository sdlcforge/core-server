...
That is 5 registered endpoints — the same set, at the same paths, that `@liquid-labs/liq-orgs` served before the absorption. **All 5 were migrated non-functional from `@liquid-labs/liq-orgs` and have since been repaired (see [Repaired defects](#repaired-defects-orgs-submodule) below).**

#### Repaired defects (`orgs` submodule)

Every endpoint `orgs` contributes was broken at runtime when absorbed, inherited unchanged from `@liquid-labs/liq-orgs`. Both defect classes have since been fixed, tracked as `sdlcforge/dev-core` `plan/followups.yaml` id `jY7C`:

- **`list`, `parameters-detail`, `parameters-list`, `parameters-set`** each threw `TypeError: Cannot read properties of undefined (reading 'orgs')` on their first request. Each read a `model` argument directly (`Object.values(model.orgs)`) or via `getOrgFromKey({ model, ... })` from `@liquid-labs/liq-handlers-lib` (`model.orgs[orgKey]`), but `plugable-express`'s loader never passes a `model` argument to plugin handlers — only `{ npmName, handlers, reporter, setupData, cache }` — so `model` was always `undefined`. Each handler now resolves the org through `src/orgs/handlers/_lib/get-org.mjs`, an in-package helper that reads `app.ext._liqOrgs.orgs[orgKey]` directly and throws a 404 for an unknown key (see [The `app.ext._liqOrgs` contract](#the-appext_liqorgs-contract) below); `list` reads the registry directly via `Object.values(app.ext._liqOrgs.orgs)` since it needs every org, not one.

  **Correction — the original defect note was wrong about what fixing this required.** It claimed a real fix "requires changing `getOrgFromKey`'s signature in `@liquid-labs/liq-handlers-lib`, a package outside this consolidation's scope." It does not: `getOrgFromKey`'s entire contribution is a one-line map lookup plus a 404, which `get-org.mjs` now does in-package. Nothing outside `dev-core` needed to change.
- **`create`** used to never send a response: its handler created the org's local data directory (`fs.mkdir`) and then fell off the end of the function at a bare `// TODO`, so a request hung until the client timed out. It now validates `localDataRoot` for containment under the playground root (`app.ext._liqProjects.playgroundPath`) before creating anything, rejecting an escaping path with a 400, and sends a real response via `httpSmartResponse`.

Each of the 5 handler source files' inline `KNOWN BROKEN` comments, added during the absorption, have been removed now that the defects they described no longer exist.
...
- **`app.ext._liqOrgs.orgs`** — a `key` → `Organization` map, the org registry. **Written** by `orgs`' own deferred `load orgs` setup method; **read** by `orgs`' own `list`, `parameters-list`, `parameters-detail`, and `parameters-set` handlers (via `src/orgs/handlers/_lib/get-org.mjs`, or directly in `list.mjs`), and by `@sdlcforge/core-server`'s in-tree `controls` submodule in three modules (`src/controls/resources/load-controls.mjs`, `src/controls/integrations/get-question-controls.mjs`, and `src/controls/handlers/orgs/controls/_lib/list-lib.mjs`) — the in-tree successor to the `@liquid-labs/liq-controls` package, now deprecated in favor of `core-server`.
