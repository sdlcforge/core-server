...
- **The one *guarded* hook, `integrationHook:controls/getQuestionControls`, is declared — as `optional: true` with a `reason`.** `src/work/handlers/_lib/submit-lib.mjs:94` gates the call behind `hasHook` and degrades gracefully, so `optional: true` is a true statement about the code rather than a severity dodge; an unsatisfied optional reports at `info`.

Three declared requirements are *expected* to report unsatisfied until the packages on their other side are manifested, and that is the intended product rather than a defect: `appExt:credentialsDB` (from `projects` at `load` and `work` at `runtime`, provided by `core-server`'s in-tree `src/credentials/`) and `appExt:_liqOrgs.orgSetupMethods` (from `orgs` at `setup`, written by `liq-policy`). Everything else — every framework-provided capability and every intra-package edge — resolves clean against the framework's own intrinsic manifest today.

**The `credentialType:GITHUB_API` naming commitment.** `src/projects/setup.mjs:8` calls `setupCredentials()` from `@liquid-labs/credentials-db-plugin-github`, registering a `GITHUB_API` credential *type* onto the shared `credentialsDB` object; `liq-integrations-issues-github`, outside this package, later reads it via `credentialsDB.getToken('GITHUB_API')`. This is not an `app.ext` key and must never be spelled `appExt:credentialsDB.GITHUB_API` — the schema's dotted-member rule is about a member of an `app.ext` container, which this is not. It falls instead under the schema's open/vendor kind tier, which names this exact case as its motivating example. dev-core declares it as a `provides` on `projects` at `load` phase under the kind name **`credentialType`**, giving the full capability string:

```text
credentialType:GITHUB_API
```

An unmatched `provides` produces no finding at all, so declaring it is risk-free — but it also closes nothing on its own. Capability matching is exact and case-sensitive, with no wildcards, so **the requiring half, whenever it is written, must use this identical string** for the coupling to become checkable. `credentialType` was chosen over alternatives like `credential` or `credentialKind` because it names what `setupCredentials()` actually registers — a credential *type* on the DB, not a credential value — and reads unambiguously in a diagnostic line. Any package declaring `requires: credentialType:GITHUB_API` matches this provider.
