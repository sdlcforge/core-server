...
- when \`removePackages\` is set true, extra packages are removed (see parameter documentation)
- outdated packages are updated; pre-release package 'minimums' are updated while production packages are updated but the semver range spec is kept as is unless \`updateMinimums\` is set to true (see parameter documentation).`
  }
  ...
  const parameters = [
    ...
    },
    {
      name        : 'removePackages',
      isBoolean   : true,
      description : 'By default, "extra" packages are not automatically removed as there are many instances in which they are used indirectly. E.g., the "sdlc-resource" packages used to provide Babel transpiling or Jest unit testing. Setting this parameter to true will remove these packaes. You can exclude false positives (and protect them from removal) by listing known good packages which would otherwise look unused as an array of glob patterns in \'_npm-check-plus.depcheck.ignoreMatches\' in the \'package.json\' file.'
    },
    {
      name        : 'updateMinimums',
      isBoolean   : true,
      description : 'By default production package semver range specifications are left in place. Setting `updateMinimums` true will make the current \'wanted\' package the new minimum. Note, pre-production package minimums are always updated.'
    }
  ]
