...
3. **Change nothing else.** `name`, `main`, `scripts`, `engines`, `license`, `repository`, `bugs`, `homepage`, `dependencies`, `devDependencies`, and the `liq` metadata block all stay exactly as they are. Do **not** add `"private": true` — this package must remain publishable, since a final labelled release is the point.
4. **`make qa` green** (test and lint) after the edit.

## Validation

- `git diff package.json` shows changes on exactly two lines: `version` and `description`. Anything else in the diff is a defect.
