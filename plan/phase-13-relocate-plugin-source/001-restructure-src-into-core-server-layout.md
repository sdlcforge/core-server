...
All 12 files found under `src/` (matching the task's expected-mapping table exactly, including the two Phase 12 `determine-current-milestone` files — Phase 12 had landed) were relocated via `git mv` into `src/integrations-issues-github/`, preserving internal structure (`test/` nested under the new root). `git status` after the moves showed all twelve as `R100` renames; `git log --follow` on both a `.mjs` file (`create-or-update-pull-request.mjs`) and the new `index.js` reaches pre-move history through the rename. `src/index.js` was then recreated as the single-line thin re-export `export * from './integrations-issues-github'` plus trailing newline (45 bytes total, confirmed via `wc -c`); `src/integrations-issues-github/index.js` is byte-identical to the pre-move `src/index.js` (diffed against a copy taken from commit `4c05be3`, the pre-move baseline — zero diff).

No `Makefile` change was needed, confirmed empirically: `make build` is green post-move and produces `dist/liq-integrations-issues-github.js`, which is **byte-identical** to the pre-move build output (a plain `diff` against a pre-move copy of the bundle showed zero differences — better than the "only path/source-map artifacts differ" expectation, since Rollup's bundling collapses the directory-structure difference entirely). `make test` passes at the pre-move baseline: 2 suites / 5 tests, unchanged, with both relocated test files confirmed to transpile into and execute from `test-staging/integrations-issues-github/test/…` (not silently skipped). `make lint` passes clean. A full-tree import scan (`grep -rn "from '\.\./\.\./" src/`, plus manual review of every `import` line under `src/`) confirms no import escapes the moved subtree; every relative import remained a same-directory `'./…'` (or `'../…'` inside `test/`) reference exactly as the task predicted.

Provider/hook parity confirmed by direct read of the relocated `src/integrations-issues-github/index.js`: exactly two `register()` calls (`providerFor: 'tickets'` and `'pull request'`), `providerTest: usesGitHubIssues` on both, seven hook registrations total (`getCurrentIntegrationUser` in both), `npmName: '@liquid-labs/liq-integrations-issues-github'` on both, `name` still absent from both (the known defect, deliberately preserved, per requirement 5 — not fixed). Setup-method parity confirmed: single `setupMethods.push` with `name: 'register github issues integrations'` and `deps: ['setup integrations']`, byte-identical to baseline. `git diff 4c05be3..HEAD -- package.json Makefile README.md AGENTS.md docs/` is empty — none of those were touched. Working tree is clean.

**Branch reachability (requirement 6 — no branch surgery performed):** the relocated `src/integrations-issues-github/…` tree currently lives only on this task's own branch, `plan/core-server-domain-consolidation-13-001` (2 commits ahead of its branch point at `plan/core-server-domain-consolidation`'s `4c05be3`: `8fa1d3a` the `git mv`, `422831c` the `src/index.js` re-export). It is **not yet** reachable from `plan/core-server-domain-consolidation` itself — that branch sits at `4c05be3` pending this task's normal merge-back. Phase 12's inlining (`determine-current-milestone.mjs` and its test) **is** already reachable from `plan/core-server-domain-consolidation` (and therefore from this task branch too, as its ancestor) — it landed via the `plan/core-server-domain-consolidation-12-001`/`-12-002` task merges before this task branch was cut. It is **not** reachable from `main`, which — per the source inventory's anomaly 5 — sits 3 commits ahead of the `plan`/`main` merge-base on an unrelated (README/AGENTS/docs authoring) line of work and carries neither Phase 12's inlining nor this relocation. No branch surgery was performed; this is a report for the dispatching manager to reconcile before `core-server`'s absorb task is dispatched, per the task's own instruction.

**Validation summary:** all checks in `## Validation` passed. See the structured report for the itemized list.

**Files touched:** `src/index.js` (content edit, only content change); 11 other files relocated via `git mv` with no content change (see requirement 2's table for the full old→new mapping).
