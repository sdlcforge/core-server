...
7. **Document the subsystem — this is authoring, not porting.** Unlike liq-projects, liq-orgs has no README, no `docs/`, and an empty `description`; the `/orgs` route surface and the `app.ext._liqOrgs` contract have never been written down anywhere. Add to dev-core's `README.md` and/or `docs/` (follow whatever structure phase 1 and the `liq-projects` absorb established):
   ...
   - **The known defects, disclosed rather than buried.** dev-core is inheriting five endpoints that do not work: four throw `TypeError: Cannot read properties of undefined (reading 'orgs')` on first request because they read a `model` argument `plugable-express` no longer passes to plugin handlers, and `create` never sends a response. Migrating them unchanged is the deliberate D11-compliant choice; shipping them silently undocumented is not. State it plainly and link the follow-up (a `sdlcforge/dev-core` followup, `plan/followups.yaml` id `jY7C`, already tracks fixing these).

7a. **Also add an inline code comment to each of the 5 affected handler files**, by manager decision — README-level documentation (requirement 7) is not sufficient on its own; a reader of the source itself must see the defect without cross-referencing docs. This is the one deliberate exception to requirement "no content drift" below: only comment lines are added, no executable line changes.
   - `src/orgs/handlers/list.mjs`, `src/orgs/handlers/parameters-detail.mjs`, `src/orgs/handlers/parameters-list.mjs`, `src/orgs/handlers/parameters-set.mjs`: immediately above the line(s) that read `model` (directly, or via `getOrgFromKey({ model, ... })`), add a comment in the shape of:
     ```js
     ...
     ```
   - `src/orgs/handlers/create.mjs`: immediately above the trailing `// TODO` where the function falls off without responding, add:
     ```js
     ...
     // Tracked: sdlcforge/dev-core plan/followups.yaml id jY7C.
     ```
   Adapt exact wording/placement to each file's actual structure — the content above is the substance to preserve, not a literal template. Do not alter any executable line while adding these comments.
...
- **No content drift beyond the five permitted comment additions.** `git diff` between `liq-orgs/<branch>`'s `src/orgs/**` and dev-core's `src/orgs/**` after the merge shows changes in **exactly** `list.mjs`, `parameters-detail.mjs`, `parameters-list.mjs`, `parameters-set.mjs`, and `create.mjs` — each a comment-only addition per requirement 7a — and nothing else. No executable line differs anywhere under `src/orgs/`. If you find yourself editing any other file, or changing an executable line in any of these five, stop and justify it in the report.
- **The five inline notes are actually present and substantive.** Each of the five files named in requirement 7a carries a comment naming the specific defect (the `TypeError`/`model` issue for the four, the never-responds issue for `create`) and the followup id `jY7C`. Not a generic "known issue" placeholder — a reader of the source alone should understand the failure mode without opening any other document.
...
- **The four dead handlers and the `create` stub migrate as-is** (D11). Documenting them in dev-core's README/docs is required (requirement 7); documenting them inline in each affected source file is also required (requirement 7a, a manager-directed exception to the otherwise-strict no-content-drift rule); fixing them is out of scope and would require changing `getOrgFromKey` in `@liquid-labs/liq-handlers-lib`, outside this plan-group. The `sdlcforge/dev-core` followup tracking the actual fix is `plan/followups.yaml` id `jY7C`.
