...
1. **Move the tree with `git mv`** (so rename detection and `--follow` work), per this mapping:
   ...
   - `src/handlers/projects/` → `src/projects/handlers/` (the whole subtree in one move: 17 handler modules including `document.js`, `index.js`, `_lib/` with its 13 modules and `_lib/test/` with 2 test files and 7 data fixtures, `test/` with its 6 `.test.mjs` files, the stray `close-implied.mjs`, and `test/lib/`'s 2 helpers, and `releases/` with its 2 handlers, `index.js`, and 3 `_lib/` modules)
