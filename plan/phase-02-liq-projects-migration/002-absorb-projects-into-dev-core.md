...
2. **Resolve root-level paths in dev-core's favor, and drop donor package-level files.** Expected conflicts (dev-core's version wins, byte-for-byte): `package.json` (before the dependency union in requirement 3), `package-lock.json`, `.gitignore`, `Makefile`, `make/10-locations.mk`, `make/10-resources.mk`, `make/15-data-finder.mk`, `make/20-js-src-finder.mk`, `make/55-lint.mk`, `make/55-test.mk`, `make/95-final-targets.mk`, `.sdlc-data.yaml`, `README.md`. Expected clean arrivals that must be removed with `git rm`: `src/index.js` (liq-projects's thin re-export — dev-core's `src/index.mjs` supersedes it), `make/50-liq-projects-js.mk`, `.catalyst-data.yaml`, **and `plan/manifest.yaml` plus any `plan/plan-summary-*.md`** — liq-projects's own Flow planning artifacts. These last ones arrive as a clean (non-conflicting) merge, not a conflict, so they will NOT prompt a manual look the way the conflicting files do — check for and remove them explicitly rather than relying on the merge to surface them. Verify the actual conflict/arrival set against this list and report any difference; do not silently accept an unexpected file. After the merge, **also explicitly confirm `src/index.mjs` still contains dev-core's own aggregator** (not overwritten by a donor's re-export) — this check matters less for this donor (liq-projects's root entry is `src/index.js`, a different filename) but is the same check every subsequent absorption must repeat, and getting it right here sets the precedent.
...
- **No donor package-level file survived.** All of these return nothing: `git ls-files | grep -x '.catalyst-data.yaml'`, `git ls-files | grep -x 'src/index.js'`, `git ls-files | grep 'make/50-liq-projects-js.mk'`, `git ls-files | grep -x 'plan/manifest.yaml'`, `git ls-files | grep 'plan/plan-summary-'`. `git ls-files Makefile make .sdlc-data.yaml .gitignore plan` shows exactly dev-core's own set (no `plan/` directory at all, since dev-core is not itself Flow-managed).
- **Aggregator not overwritten.** `git show HEAD:src/index.mjs` (post-merge-resolution) still contains dev-core's own aggregator logic — a `handlers` array built from submodule imports, not a single-submodule re-export.
