...
- Subject to the release decision in [`plan/notes/scope-confirmation.md`](../notes/scope-confirmation.md): a `@sdlcforge/core-server` version bump and an attempted `npm publish`, with the exact command handed to the user verbatim if the environment blocks it — this is what unblocks each donor's own retirement phase.
