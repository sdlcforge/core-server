...
- A decision, recorded beside the declaration, on the two `integration:` provides that carry `providerTest: usesGitHubIssues`. A static `provides` asserts them unconditionally, which over-claims: the graph closes and `No provider found for 'tickets'` still fires at request time for a non-GitHub project. Upstream raised this as a gap and its proposed `conditional:` marker was not adopted into the decided grammar, so the choice here is between an accepted, documented over-claim and omitting those provides. Either is defensible; an undocumented one is not.
