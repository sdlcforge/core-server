...
`dev-core` owns, and authors exactly once, every package-level file: `package.json` (which, since the plugin manifest work, also carries the top-level `"plugable"` block described in [`app.ext` contract freeze](#appext-contract-freeze) and [Publishing hygiene](#publishing-hygiene) below — the manifest lives inside this existing root file rather than adding a new one), `package-lock.json`, `Makefile`, `make/*.mk`, `.sdlc-data.yaml`, `.gitignore`, `README.md`, `docs/`, and `src/index.mjs`.

An absorption never carries a source package's root files in. At absorb time, the following source-package files are dropped, because `dev-core`'s own version of each already wins: `package.json`, `package-lock.json`, `Makefile`, `make/`, `.gitignore`, `README.md`, `.sdlc-data.yaml`, `.catalyst-data.yaml`, any generated `docs/*.html`, the source package's root `src/index.*` (**but read the hazard below before running `git rm` on it** — for a source package whose root entry is `src/index.mjs`, "dropped" means *resolving a conflict in `dev-core`'s favor*, never `git rm`), the source package's trivial `src/handlers/index.*` re-export, and **every file the source repository carries under `plan/`** — its own Flow planning tree.
...
**A specific hazard worth calling out by name, and the one place the drop-list phrasing above must not be taken literally:** for one source package (`plugable-projects-audit`), the source's own root entry point is literally named `src/index.mjs` — identical to `dev-core`'s own aggregator path. Every other source package's root entry is `src/index.js`, which arrives as a clean, non-conflicting file to `git rm`; this one instead arrives as an **add/add merge conflict on the path `dev-core`'s aggregator occupies**, and the resolution is:

```bash
git checkout --ours -- src/index.mjs && git add src/index.mjs
```

**Never `git rm src/index.mjs`, and never take `--theirs`** — either one deletes `dev-core`'s aggregator outright or replaces it with the source package's one-line `export * from './projects-audit'`. Because that replacement is still a valid module, `make build` stays green and `dist/dev-core.js` still exports a plausible `handlers` array; what is silently gone is every other submodule's handlers *and* the entire composite `setup`, so the merged plugin stops registering GitHub credentials, stops creating the playground directory, stops publishing `app.ext._liqProjects` and `app.ext._liqOrgs`, and stops registering every path var — surfacing much later, in a different repository, as a startup crash with no obvious cause.

**After every merge, whoever performs the absorption must verify that `src/index.mjs`'s content is still `dev-core`'s own aggregator — not a source package's re-export — before considering the merge step complete.** The check that actually proves it is a blob comparison against the pre-merge commit, run *before* the aggregator is edited to wire the new submodule in: `git diff <pre-merge-commit> -- src/index.mjs` must be empty at that point. A `typeof setup === 'undefined'` on the built bundle is the corresponding after-the-fact signal that the aggregator was replaced. (Verified in practice: this is exactly what the `projects-audit` absorption's merge produced, and resolving it `--ours` is what kept the other three submodules wired.)
...
5. Union the source package's runtime `dependencies` into `dev-core`'s `package.json` — on any overlapping dependency, take the higher version range and record the choice — then refresh `package-lock.json`.
   ...
   **A `file:` spec is not a range and has no place in this union.** "Take the higher range" presumes two comparable ranges; a `file:.yalc/…` spec has no ordering to compare and does not survive publication, so unioning one in would make `@sdlcforge/dev-core` uninstallable for anyone but the developer who created that local directory. When a source package declares one, take `dev-core`'s registry range (or the source package's own published registry range, once its own remediation has landed) and verify afterwards that `grep -n 'file:' package.json` returns nothing.
...
`dev-core`'s `setup` export is `async` and awaits each submodule's own setup, in this fixed order: `projects`, then `orgs`, then `work`. (`projects-audit` has no `setup` at all — handlers only, and it is absent from the ordered list rather than represented there by a placeholder or a no-op.)

Having no `setup` is not the same as having no dependency. `projects-audit`'s handlers read `app.ext._liqProjects.playgroundMonitor` at request time, and two of its four routes carry a `:projectName` segment — a path variable only `projects`' `setup` registers, and one `plugable-express`'s `pathToRe` **throws** on (`Unknown variable path element type 'projectName' …`) when it is unregistered. So this submodule depends on `projects` at *handler-registration* time as well as at request time, even though it contributes nothing to the ordering below. Nothing breaks in practice because `plugable-express`'s `load-plugins.js` runs every plugin's `setup` eagerly and defers all handler registration until afterwards — but a consumer that drops `liq-projects` while still loading `plugable-projects-audit` gets exactly that throw at startup.

The ordering is not arbitrary: `projects`' setup is eager — it registers GitHub credentials and installs `app.ext._liqProjects = { playgroundMonitor, playgroundPath }` before returning — while `orgs` defers its own work onto `app.ext.setupMethods` (run later by the server) but still needs `app.ext._liqProjects` to already exist by the time that deferred work runs, and `work` only needs `app.ext.serverConfigRoot`, which the framework itself supplies at server initialization regardless of submodule order. Running `projects` first is therefore load-bearing, not a stylistic default; `work`'s third position, by contrast, is a fixed convention rather than a dependency it satisfies — nothing in `work`'s setup requires `projects` or `orgs` to have run, so a passing composite-setup smoke test must not be read as evidence of an inter-submodule ordering requirement for it. `orgs`' and `work`'s setups are both synchronous and return `undefined`; the composite `await` is a harmless no-op for each.

`registerPathVar` is forwarded to each submodule's setup unchanged. The merged set of path variables these four submodules register — `projectName`, `newProjectName`, `orgKey`, `newOrgKey`, `parameterKey`, `workKey` — has no name collisions. That matters because a collision would not be shadowed silently: `registerPathVar` **throws** `Path variable '<name>' is already registered.` on the second registration, crashing server startup. Verified as of the `work` absorption, against the built bundle: the five names registered from `setup` (`newProjectName`, `projectName`, `newOrgKey`, `orgKey`, `workKey`) plus the one registered at handler-registration time (`parameterKey`) are each registered exactly once. `parameterKey` is registered not from `orgs`' `setup` but from its `parameters-detail` handler's `func`, at route-registration time — a *handler* can register a path variable there too, so the merged path-variable surface is not fully determined by reading each submodule's `setup` function alone.
...
This is precisely why the plugin manifest (see [`app.ext` contract freeze](#appext-contract-freeze) and [`README.md`'s "The plugin manifest"](../README.md#the-plugin-manifest)) is authored as a `package.json` `"plugable"` block rather than a root `plugable.yaml`: a root file not named in the eventual `files` allowlist above is silently dropped from the published tarball — no error, anywhere, ever — leaving the published package indistinguishable from an unmanifested plugin. `package.json` itself cannot be excluded by its own `files` field, so the block form survives that future unconditionally. Keep this link in mind before "simplifying" the manifest into a standalone `plugable.yaml`: doing so would work today, while this package still ships no allowlist, and silently stop working the moment one lands.

## Scope fences

This consolidation does not change behavior, routes, or `app.ext` names beyond what collapsing four packages into one unavoidably requires (the `npmName` provenance change described in [Plugin contract](#plugin-contract)). It does not upgrade any dependency beyond the version-range union described in [Absorption recipe](#absorption-recipe). And it does not absorb `liq-projects-lib`, which stays exactly where it is. `crossLinkDevProjects` — that package's one function the `work` submodule's source package ever used, and never `projects` — was inlined into `liq-work` (together with its only test) before the `work` absorption, so `dev-core` carries its own copy at `src/work/handlers/_lib/cross-link-dev-projects.mjs` and declares no dependency on `@liquid-labs/liq-projects-lib` at all. The inlining was dependency-neutral: the function's only non-builtin imports (`@liquid-labs/federated-json`, `@liquid-labs/shell-toolkit`) were already `work` dependencies.
