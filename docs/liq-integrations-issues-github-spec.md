...
- **Milestone determination is an external dependency, not yet inlined.** `createOrUpdatePullRequest` currently imports `determineCurrentMilestone` from `@liquid-labs/liq-projects-lib` to assign a milestone when creating a PR. This is a known external dependency; a separate, not-yet-executed plan-group item is expected to inline this function into this repository in the future. This spec documents current, as-shipped behavior — the external import — and does not assume the inlining has happened.
- **Dependent on the broader `liq`/`liquid-labs` toolkit libraries.** The plugin relies on `@liquid-labs/git-toolkit` (origin/main resolution, shell push), `@liquid-labs/github-toolkit` (GitHub login and org/repo resolution from `package.json`), `@liquid-labs/liq-qa-lib` (QA file link resolution), `@liquid-labs/octocache` (cached GitHub API client), and `@liquid-labs/shell-toolkit` (shell command execution) as external dependencies, all of type-checked against the versions pinned in `package.json`.
