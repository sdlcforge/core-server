...
- **Built-in capability is attributed to `@sdlcforge/core-server`'s own package identity.** Not every capability arrives as a separate npm-dependency package: policy controls, credential management, GitHub issue-tracking integration, project lifecycle management, organization settings, unit-of-work orchestration, and project auditing are all `core-server`'s own built-in (in-tree) submodules — seven components in total, registered as one plugin under the host package's identity. `GET /server/plugins/list`, `GET /server/plugins/integrations/list`, and the framework's `GET /server/plugins/:serverPluginName/details` all attribute this capability to `@sdlcforge/core-server` itself rather than to a separate package name.
