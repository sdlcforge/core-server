...
- **yalc for local unpublished-dependency development.** The project accepts a `.yalc/`-based local-linking workflow over a full publish/install cycle for two dependencies: `@liquid-labs/plugable-express`, which `core-server` depends on so heavily that active parallel development needs it linked rather than published-and-installed on every iteration, and `@sdlcforge/dev-core`, which is not yet published to npm and so has no other way to be installed as a dependency until it is. The day-to-day commands for this workflow live in `AGENTS.md`, not here. Under Bun this carries a consequence npm never had: once `bun.lock` holds a resolved entry for a `file:.yalc/…` specifier, a bare `bun install` re-copies the linked package's content but does not re-resolve its own dependency list, so it silently leaves a newly-added transitive dependency unresolved while still reporting success. A `yalc push` that changed either linked package's `dependencies` therefore requires `rm -f bun.lock && bun install` (or `scripts/provision-local-deps.sh --refresh-lock`) to pick the change up.
